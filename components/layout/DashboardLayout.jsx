...
export default function DashboardLayout({ children }) {
  ...
  // Tutup mobile sidebar kalau resize ke desktop
  useEffect(() => {
    const handleResize = () => {
    ...
    <div className="flex flex-col h-full">
      ...
      <div className={`h-16 flex items-center border-b border-slate-800/60 shrink-0 bg-slate-950/20
        ...
        {(!collapsed || isMobile) && (
          <div className="flex items-center gap-2.5 overflow-hidden">
            <div className="h-8 w-8 bg-blue-600 rounded-lg flex items-center justify-center font-bold text-lg shrink-0 shadow-lg shadow-blue-900/40">
              E
            </div>
        ...
        {collapsed && !isMobile && (
          <div className="h-8 w-8 bg-blue-600 rounded-lg flex items-center justify-center font-bold text-lg shadow-lg shadow-blue-900/40">
            E
          </div>
        )}
        ...
        {!isMobile && (
          <button
            ...
            className={`h-7 w-7 rounded-md bg-slate-800 hover:bg-slate-700 flex items-center justify-center text-slate-400 hover:text-white transition-all duration-200 shrink-0
              ${collapsed ? "absolute -right-3.5 top-4 shadow-md border border-slate-700/50 z-10" : ""}`}
        ...
        {isMobile && (
          <button
            ...
            className="h-7 w-7 rounded-md bg-slate-800 hover:bg-slate-700 flex items-center justify-center text-slate-400 hover:text-white"
          >
            <X size={14} />
          </button>
        )}
      ...
      <nav className="flex-1 overflow-y-auto py-4 scrollbar-thin scrollbar-thumb-slate-700 scrollbar-track-transparent">
        <div className={`space-y-0.5 ${collapsed && !isMobile ? "px-2" : "px-3"}`}>
          ...
          <SidebarItem
            icon={<Map size={17} />}
            label="Sebaran & Infografis"
            active={pathname === "/sebaran"}
          ...
          {collapsed && !isMobile ? (
            <div className="space-y-0.5">
              <Tooltip label="Database">
                <button
                  ...
                  className={`flex w-10 mx-auto items-center justify-center rounded-lg py-2.5 transition-all duration-200
                    ${(pathname === "/ptk" || pathname === "/diklat")
                      ? "bg-blue-600 text-white shadow-md shadow-blue-900/30"
                      : "text-slate-400 hover:bg-slate-800/80 hover:text-white"}`}
                >
                  <Database size={17} />
              ...
              {isDatabaseOpen && (
                <div className="space-y-0.5 mt-1">
                  <Tooltip label="Data PTK">
                    <button
                      onClick={() => navigate("/ptk")}
                      className={`flex w-10 mx-auto items-center justify-center rounded-lg py-2 text-[11px] font-bold transition-all duration-200
                        ${pathname === "/ptk" ? "bg-blue-600/80 text-white" : "text-slate-500 hover:bg-slate-800 hover:text-white"}`}
                    >
                      P
                    </button>
                  </Tooltip>
                  <Tooltip label="Data Diklat">
                    <button
                      onClick={() => navigate("/diklat")}
                      className={`flex w-10 mx-auto items-center justify-center rounded-lg py-2 text-[11px] font-bold transition-all duration-200
                        ${pathname === "/diklat" ? "bg-blue-600/80 text-white" : "text-slate-500 hover:bg-slate-800 hover:text-white"}`}
                    >
                      D
                    </button>
                  </Tooltip>
            ...
            <div>
              <button
                onClick={() => setDatabaseOpen(!isDatabaseOpen)}
                className={`flex w-full items-center justify-between px-3 py-2.5 rounded-lg transition-all duration-200 group
                  ${(pathname === "/ptk" || pathname === "/diklat") && !isDatabaseOpen
              ...
              >
                ...
                <ChevronDown
                  size={14}
                  className={`transition-transform duration-300 ${isDatabaseOpen ? "rotate-180" : ""}`}
                />
              </button>
              ...
              <div className={`overflow-hidden transition-all duration-300 ease-in-out ${isDatabaseOpen ? "max-h-32 opacity-100" : "max-h-0 opacity-0"}`}>
                <div className="ml-4 mt-1 mb-1 space-y-0.5 border-l-2 border-slate-800 pl-3">
                  {[
                    { label: "Data PTK", path: "/ptk" },
                    { label: "Data Diklat", path: "/diklat" },
                  ].map(({ label, path }) => (
                    <button
                      key={path}
                      onClick={() => navigate(path)}
                      className={`flex w-full items-center gap-2 px-3 py-2 text-sm font-medium rounded-md cursor-pointer transition-all duration-150
                        ${pathname === path
                          ? "bg-blue-600/15 text-blue-400 border border-blue-600/20"
                          : "text-slate-500 hover:text-white hover:bg-slate-800/70"}`}
                    >
                      <span className={`h-1.5 w-1.5 rounded-full shrink-0 ${pathname === path ? "bg-blue-400" : "bg-slate-700"}`} />
                      {label}
                    </button>
                  ))}
                </div>
      ...
      <div className="h-[60px] border-t border-slate-800/60 bg-slate-950/20 shrink-0 flex items-center px-3">
        <div 
          onClick={handleLogout} // PASANG DI SINI
          className={`flex items-center gap-3 rounded-lg px-2 py-2 hover:bg-red-500/10 transition-colors cursor-pointer group w-full 
          ${isLoggingOut ? "opacity-50 cursor-not-allowed" : ""}`}
        >
          <Tooltip label={collapsed && !isMobile ? "Admin User" : null}>
            <Avatar className="h-8 w-8 shrink-0 ring-2 ring-slate-700 group-hover:ring-blue-500 transition-all">
              <AvatarImage src="" />
              <AvatarFallback className="bg-blue-700 text-white text-xs font-bold">AU</AvatarFallback>
            </Avatar>
          </Tooltip>
          {(!collapsed || isMobile) && (
            <>
              <div className="flex-1 overflow-hidden">
                ...
                <p className="text-xs text-slate-500 truncate leading-tight">admin@bbgtk.id</p>
              </div>
  ...
  return (
    <div className="flex h-screen w-full bg-slate-100 font-sans overflow-hidden">
      ...
      {/* ===== SIDEBAR DESKTOP ===== */}
      <aside
        className={`hidden md:flex flex-col bg-slate-900 text-white shrink-0 relative
      ...
      <aside
        className={`fixed top-0 left-0 h-full w-72 bg-slate-900 text-white flex flex-col z-50
          md:hidden transition-transform duration-300 ease-in-out shadow-2xl
        ...
        <header className="h-16 shrink-0 bg-white border-b border-slate-200/80 flex items-center justify-between px-4 md:px-6 gap-4 shadow-sm">
          ...
          <div className="flex items-center gap-3 min-w-0">
            <button
              ...
              className="md:hidden h-9 w-9 rounded-lg hover:bg-slate-100 flex items-center justify-center text-slate-500 transition-colors"
            >
              <Menu size={20} />
            </button>
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <div className="relative hidden sm:flex items-center">
              ...
              <input
                ...
                className="h-9 pl-9 pr-4 rounded-lg bg-slate-100 border border-transparent focus:border-blue-500 focus:bg-white outline-none text-sm w-44 focus:w-56 transition-all duration-300 text-slate-700 placeholder:text-slate-400"
              />
            </div>

            {/* Notifikasi */}
            <button className="relative h-9 w-9 rounded-lg hover:bg-slate-100 flex items-center justify-center text-slate-500 hover:text-slate-700 transition-colors">
              <Bell size={18} />
            ...
            <div className="flex items-center gap-2 pl-1 border-l border-slate-200 ml-1">
              <Avatar className="h-8 w-8 cursor-pointer ring-2 ring-transparent hover:ring-blue-500 transition-all">
                <AvatarImage src="" />
                <AvatarFallback className="bg-blue-600 text-white text-xs font-bold">AU</AvatarFallback>
              </Avatar>
