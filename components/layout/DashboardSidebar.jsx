...
export default function DashboardSidebar({
  ...
  return (
    <div className="flex flex-col h-full relative bg-slate-900">
      {/* Logo Section */}
      <div
        className={`h-16 flex items-center border-b border-slate-800/60 shrink-0 bg-slate-950/20 ${collapsed && !isMobile ? "justify-center" : "px-4 justify-between"}`}
      >
        {(!collapsed || isMobile) && (
      ...
      <nav className="flex-1 overflow-y-auto py-4 scrollbar-hide overflow-x-hidden">
        <div
          className={`space-y-0.5 ${collapsed && !isMobile ? "px-2" : "px-3"}`}
        >
          ...
          <div className="space-y-0.5">
            ...
            {!collapsed && (
              <div
                className={`overflow-hidden transition-all duration-300 ${isDatabaseOpen ? "max-h-40 opacity-100 mt-1" : "max-h-0 opacity-0"}`}
              >
                <div className="ml-4 space-y-0.5 border-l-2 border-slate-800 pl-3">
                  <button
                    onClick={() => navigate("/ptk")}
                    className={`flex w-full items-center gap-2 px-3 py-2 text-xs rounded-md transition-all ${pathname === "/ptk" ? "text-blue-400 font-bold bg-blue-600/5" : "text-slate-500 hover:text-slate-200"}`}
                  >
                    <span
                      className={`h-1.5 w-1.5 rounded-full ${pathname === "/ptk" ? "bg-blue-400 shadow-[0_0_8px_rgba(96,165,250,0.6)]" : "bg-slate-700"}`}
                    />{" "}
                    Data PTK
                  </button>
                  <button
                    onClick={() => navigate("/diklat")}
                    className={`flex w-full items-center gap-2 px-3 py-2 text-xs rounded-md transition-all ${pathname === "/diklat" ? "text-blue-400 font-bold bg-blue-600/5" : "text-slate-500 hover:text-slate-200"}`}
                  >
                    <span
                      className={`h-1.5 w-1.5 rounded-full ${pathname === "/diklat" ? "bg-blue-400 shadow-[0_0_8px_rgba(96,165,250,0.6)]" : "bg-slate-700"}`}
                    />{" "}
                    Data Diklat
                  </button>
                </div>
          ...
          {!collapsed && (
            <div className="flex-1 overflow-hidden flex justify-between items-center">
              <div className="overflow-hidden">
                ...
                <p className="text-[10px] text-slate-500 truncate group-hover:text-red-400 transition-colors">
                  Logout Aplikasi
                </p>
              </div>
              <LogOut
                size={14}
                className="text-slate-600 group-hover:text-red-400 transition-colors shrink-0"
              />
            </div>
          )}
        </button>
