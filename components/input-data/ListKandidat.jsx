...
export default function ListKandidat({ diklatId, diklatTitle, onSuccess }) {
  ...
  const [kandidatToDelete, setKandidatToDelete] = useState(null);
  ...
  const handleSaveToAlumni = async () => {
    ...
    setIsSaving(true);
    try {
      const res = await fetch(`/api/diklat/${diklatId}/alumni`, { headers: { }, 
        method: 'POST',
      });
  ...
  const confirmDelete = async () => {
    if (!kandidatToDelete) return;
    setKandidatToDelete(null);

    try {
      const res = await fetch(`/api/diklat/${diklatId}/kandidat?kandidat_id=${kandidatToDelete}`, { headers: { },  method: 'DELETE' });
      if (res.ok) {
  ...
  return (
    <div className="space-y-4">
                ...
                <TableRow key={row.kandidat_id} className="hover:bg-slate-50/50">
                  ...
                  <TableCell className="text-right">
                    <Button
                      ...
                      onClick={() => setKandidatToDelete(row.kandidat_id)}
                    >
      ...
      <AlertDialog open={showAlumniDialog} onOpenChange={setShowAlumniDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Konfirmasi Pindah Alumni</AlertDialogTitle>
            <AlertDialogDescription>
              Yakin ingin memindahkan {data.length} kandidat ini ke daftar peserta alumni?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Batal</AlertDialogCancel>
            <AlertDialogAction onClick={handleSaveToAlumni} className="bg-blue-600 hover:bg-blue-700">Ya, Pindahkan</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>

      <AlertDialog open={!!kandidatToDelete} onOpenChange={(open) => !open && setKandidatToDelete(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Hapus Kandidat</AlertDialogTitle>
            <AlertDialogDescription>
              Yakin ingin menghapus guru ini dari daftar kandidat? Tindakan ini tidak dapat dibatalkan.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Batal</AlertDialogCancel>
