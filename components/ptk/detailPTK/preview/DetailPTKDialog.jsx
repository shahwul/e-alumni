...
export function DetailPTKDialog({ open, onOpenChange, nik }) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-150 bg-white p-0 overflow-hidden text-left">
            ...
            {loading ? (
              ...
              <>
                <h3 className="text-lg font-bold leading-tight">
                  {data?.profil?.nama_ptk}
                </h3>
                <div className="flex items-center justify-center sm:justify-start gap-2 text-slate-300 text-sm">
                  <span className="font-mono bg-slate-800 px-1.5 rounded text-xs">
                    {data?.profil?.nik}
                  </span>
                  <span>•</span>
                  <span className="capitalize">
                    {data?.profil?.status_kepegawaian?.toLowerCase()}
                  </span>
                </div>
              </>
            )}
        ...
        <div className="p-6">
          {loading ? (
            <div className="space-y-4 text-center text-slate-400 py-8">
              Loading data...
            </div>
          ) : (
                  ...
                  <div>
                    ...
                    <p className="text-sm font-semibold text-slate-800">
                      {data?.profil?.kecamatan
                        ? `${data?.profil?.kecamatan}, ${data?.profil?.kabupaten}`
                        : "-"}
                    </p>
                  </div>
                </div>
                              ...
                              >
                                {item.status_kelulusan === "Lulus" ? (
                                  ...
                                  <>
                                    ...
                                    {item.status_kelulusan || "Belum Lulus"}
                                  </>
                                )}
                              </Badge>
                              {item.nilai_akhir && (
