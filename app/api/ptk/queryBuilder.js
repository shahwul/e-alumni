...
export function buildPTKQueryParts(searchParams) {
  ...
  const params = {
    ...
    kabupaten: searchParams.get('kabupaten') ? searchParams.get('kabupaten').split(',') : [],
    kecamatan: searchParams.get('kecamatan') ? searchParams.get('kecamatan').split(',') : [],
  ...
  const judulArray = params.judulDiklatRaw 
    ? (params.judulDiklatRaw.includes('||') ? params.judulDiklatRaw.split('||') : params.judulDiklatRaw.split(',')) 
    : [];

  // Cek apakah ada filter diklat aktif?
  const hasDiklatFilter = judulArray.length > 0 || params.kategoriParam || params.programParam;
  ...
  if (params.search) {
    ...
    values.push(`%${params.search}%`); counter++;
  }
  if (params.jenis_kelamin) {
    whereClause += ` AND mv.jenis_kelamin = $${counter}`;
    values.push(params.jenis_kelamin); counter++;
  }
  if (params.sekolah) {
    whereClause += ` AND mv.nama_sekolah ILIKE $${counter}`;
    values.push(`%${params.sekolah}%`); counter++;
  }
  if (params.jenjang && params.jenjang !== 'ALL') {
    whereClause += ` AND mv.bentuk_pendidikan = $${counter}`;
    values.push(params.jenjang); counter++;
  }
  if (params.mapel) {
    whereClause += ` AND mv.riwayat_sertifikasi ILIKE $${counter}`;
    values.push(`%${params.mapel}%`); counter++;
  }
  if (params.usiaMin && params.usiaMax) {
    whereClause += ` AND mv.usia_tahun >= $${counter} AND mv.usia_tahun <= $${counter + 1}`;
    values.push(params.usiaMin, params.usiaMax); counter += 2;
  }
  if (params.statusKepegawaian) {
    whereClause += ` AND mv.status_kepegawaian = $${counter}`;
    values.push(params.statusKepegawaian); counter++;
  }
  if (params.jenisPTK) {
    whereClause += ` AND mv.jenis_ptk = $${counter}`;
    values.push(params.jenisPTK); counter++;
  }
  if (params.jabatanPTK) {
    whereClause += ` AND mv.jabatan_ptk = $${counter}`;
    values.push(params.jabatanPTK); counter++;
  }
  if (params.pendidikanTerakhir) {
    whereClause += ` AND mv.riwayat_pend_jenjang = $${counter}`;
    values.push(params.pendidikanTerakhir); counter++;
  }
  if (params.pendidikanBidang) {
    whereClause += ` AND mv.riwayat_pend_bidang = $${counter}`;
    values.push(params.pendidikanBidang); counter++;
  }
  if (params.pangkatGolongan) {
    whereClause += ` AND mv.pangkat_golongan = $${counter}`;
    values.push(params.pangkatGolongan); counter++;
  }
  
  // Boolean Filters
  if (params.kepalaSekolah === 'true') whereClause += ` AND mv.is_kepala_sekolah = TRUE`;
  else if (params.kepalaSekolah === 'false') whereClause += ` AND mv.is_kepala_sekolah = FALSE`;

  if (params.status === 'sudah') whereClause += ` AND mv.is_sudah_pelatihan = TRUE`;
  else if (params.status === 'belum') whereClause += ` AND mv.is_sudah_pelatihan = FALSE`;

  // Array Filters (IN clause)
  if (params.kabupaten.length > 0) {
    const placeholders = params.kabupaten.map((_, i) => `$${counter + i}`).join(',');
    whereClause += ` AND UPPER(mv.kabupaten) IN (${placeholders})`;
    params.kabupaten.forEach(k => values.push(k.toUpperCase())); counter += params.kabupaten.length;
  }
  if (params.kecamatan.length > 0) {
    const placeholders = params.kecamatan.map((_, i) => `$${counter + i}`).join(',');
    whereClause += ` AND UPPER(mv.kecamatan) IN (${placeholders})`;
    params.kecamatan.forEach(k => values.push(k.toUpperCase())); counter += params.kecamatan.length;
  }
  ...
  if (params.startDate && params.endDate && !hasDiklatFilter) {
      ...
      values.push(params.startDate, params.endDate); counter += 2;
  }
  ...
  if (hasDiklatFilter) {
    ...
    if (params.kategoriParam) subQueryConditions += ` AND md.jenis_kegiatan::text = '${params.kategoriParam}'`;
    if (params.programParam) subQueryConditions += ` AND md.jenis_program::text = '${params.programParam}'`;

    // Filter Judul
    if (judulArray.length > 0) {
      const placeholders = judulArray.map((_, i) => `$${counter + i}`).join(',');
  ...
  let orderByClause = '';
  if (params.sortParam) {
    const sortParts = params.sortParam.split(',').map(part => {
        const [field, direction] = part.split(':');
        const dir = direction === 'desc' ? 'DESC' : 'ASC';
        switch(field) {
            case 'nama': case 'nama_ptk': return `sub.nama_ptk ${dir}`;
            case 'sekolah': case 'nama_sekolah': return `sub.nama_sekolah ${dir}`;
            case 'usia': case 'usia_tahun': return `sub.usia_tahun ${dir}`;
    ...
    orderByClause = sortParts.length > 0 ? `ORDER BY ${sortParts.join(', ')}` : `ORDER BY sub.nama_ptk ASC`;
  } else {
    // Legacy Fallback
    const dir = params.sortOrderLegacy === 'desc' ? 'DESC' : 'ASC';
    switch (params.sortByLegacy) {
        case 'nama': orderByClause = `ORDER BY sub.nama_ptk ${dir}`; break;
        case 'sekolah': orderByClause = `ORDER BY sub.nama_sekolah ${dir}`; break;
        case 'status': orderByClause = `ORDER BY sub.status_kepegawaian ${dir}`; break;
        case 'usia': orderByClause = `ORDER BY sub.usia_tahun ${dir}`; break;
        default: orderByClause = `ORDER BY sub.nama_ptk ASC`;
    }
  }
